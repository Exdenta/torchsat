...
def train_one_epoch(epoch: int, dataloader: DataLoader, model, criterion: nn.Module, optimizer, device: str, writer: SummaryWriter,
    ...
    for idx, data in enumerate(dataloader):
        ...
        imc_callbacks.log_message(training_panel, imc_api.MessageTitle.LogInfo,
                                  'train-epoch:{} [{}/{}], loss: {:5.3}'.format(epoch, idx + 1, len(dataloader), loss.item()))
...
def process_preview(model, preview_imagepath: Path, channel_count: int, tile_size: int, device: str, output_dir: Path, output_filename: str, training_panel: TrainingPanelPrt) -> bool:
    ...
    try:
        ...
        image = run_seg.process_image(model=model,
                                      ...
                                      progress_bar=None)
        if image is None:
            imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogProcessingError, _(
                "Failed to process the preview image. Model training will continue"))
            return False
...
def load_data(features_dirpath: Path, labels_dirpath: Path, train_item_filenames: set, val_item_filenames: set,
    ...
    dataset_train = SegmentationDataset(features_dirpath, labels_dirpath,
                                        train_item_filenames, transforms=train_transform)
...
def train(training_panel: imc_api.TrainingPanelPrt, progress_bar: imc_api.ProgressBarPtr, current_progress: float,
    ...
    if len(mean) != len(std):
        imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogError, _(
            "The standart deviation array must be the same size as the mean array"))
        return False
    ...
    for epoch in range(epochs):
        ...
        # train
        training_metrics = train_one_epoch(epoch, train_loader, model, criterion, optimizer,
                                           device, writer, progress_step, current_progress, progress_bar, training_panel)
        if imc_callbacks.check_progress_bar_cancelled(progress_bar):
...
def continue_segmentation_training(params: imc_api.SegmentationTrainingParams, training_panel: imc_api.TrainingPanelPrt, progress_bar: imc_api.ProgressBarPtr) -> bool:
    ...
    # directory with all features splitted on tiles from prev run
    features_outpath = splitted_dataset_path / "features"
    # directory with all labels splitted on tiles from prev run
    labels_outpath = splitted_dataset_path / "labels"
    # config.json contains info about last run split configuration (to avoid unnecessary splitting)
    ...
    for path in [splitted_dataset_path, features_outpath, labels_outpath]:
        if not path.exists():
            imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogInfo, _(
                "Some directories with training data do not exist. Can't continue training!"))
            return False
    ...
    if not train_config_path.exists():
        imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogInfo, _(
            "Can't continue training! No configuration from last training were saved"))
        return False
...
def start_segmentation_training(params: imc_api.SegmentationTrainingParams, training_panel: imc_api.TrainingPanelPrt, progress_bar: imc_api.ProgressBarPtr) -> bool:
    ...
    if not train_config_path.exists():
        ...
        try:
            ...
            with open(train_config_path) as f:
                ...
                # dictionary ['item id': ('feature last update date', 'label last update date')]
                previous_dataset_items = config_json["items"]
                first_run = False
        except Exception as e:
            imc_callbacks.log_message(training_panel, imc_api.MessageTitle.LogInitError,
                                      "Failed to read dataset configuration file. Exception occured: " + str(e))
            first_run = True
    ...
    if len(splitted_features_ids_list) != len(splitted_labels_ids_list):
        imc_callbacks.log_message(training_panel, imc_api.MessageTitle.LogInfo,
                                  f"Number of feature tiles ({len(splitted_features_ids_list)}) is not the same as number of label tiles ({len(splitted_labels_ids_list)})! Trying to fix...")
        ...
        if len(intersecting_item_ids) <= 0:
            imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogError,
                                       _("No valid items in the dataset"))
            return False
    ...
    if len(train_dataset_item_ids) <= 0:
        imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogError,
                                   _("No images in the dataset for training!"))
        return False

    val_dataset_item_ids = set(list(splitted_dataset_item_ids)[dataset_split_idx:])
    if len(val_dataset_item_ids) <= 0:
        imc_callbacks.show_message(training_panel, imc_api.MessageTitle.LogError, _(
            "No images in the dataset for validation. Try adding more or changing dataset split rate"))
        return False

    imc_callbacks.log_message(training_panel, imc_api.MessageTitle.LogInfo,
                              f"Using {len(train_dataset_item_ids)} images for training, {len(val_dataset_item_ids)} images for validation")
    ...
    try:
        ...
        imc_callbacks.log_message(training_panel, imc_api.MessageTitle.LogInfo,
                                  "Unable to save training configuration file to the disk! Dataset preparation operations will repeat during next training run.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--preview_imagepath', type=str, default="",
                        help='path to the preview image to show after each epoch')
    parser.add_argument('--preview_outdir', type=str, default="", help='path to the preview images output folder')
    parser.add_argument('--mean', nargs='+', type=str,
                        help='list of means for each channel in the dataset', required=True)
    parser.add_argument('--std', nargs='+', type=str,
                        help='list of stds for each channel in the dataset', required=True)
