...
def patch_image(filepath: Path, width: int, height: int, drop_last: bool, outpath: str, colormap: bool):
    with rasterio.open(filepath, 'r') as src:
        ...
        for row in tqdm(range(rows)):
            for col in range(columns):
                outfile = os.path.join(outpath, filepath.stem + '_' + str(row) + '_' + str(col) + filepath.suffix)
...
def make_mask_cls(filepath: Path, width: int, height: int, drop_last: bool, outpath: str, colormap: bool, extension:tuple):
    ...
    if filepath.is_file():
        ...
        files = [x for x in filepath.glob('**/*') if x.suffix.lower()[1:] in extension and '._' not in str(x)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filepath', type=str, help='the target satellite image to split. Note: the file should have crs')
    ...
    parser.add_argument('--colormap', type=bool, default=False, help='weather write the colormap to the output patched image. only work for single channel image. default False.')
    parser.add_argument('--extension', type=str, default=('jpg', 'jpeg', 'png', 'tif', 'tiff'), help='the train image extension, only work for dirctory file path.')
    args = parser.parse_args()

    make_mask_cls(args.filepath, args.width, args.height, args.drop_last, args.outpath, args.colormap, args.extension)